############################################################
## Bulk RNA-seq full demo analysis
## Fig. 4 / Supplementary Fig. 4 / Supplementary Fig. 5
##
## This script uses a representative demo dataset:
##   5000 genes x 24 samples
##
...
## 3. Construct DESeq2 object and perform VST normalization
## 4. Generate six-group PCA plot
## 5. Generate Pearson correlation heatmap based on
##    top 1000 highly variable genes
## 6. Perform pairwise DESeq2 differential expression analysis
## 7. Perform three-group overall DESeq2 LRT analysis
## 8. Generate pairwise volcano plots
## 9. Generate three-group top1000 DEG heatmaps with
##    selected pathway-symbol gene labels
## 10. Perform KEGG and GO enrichment analysis
## 11. Generate selected KEGG pathway bar plots
############################################################
...
count_file <- file.path(data_dir, "gene_counts_demo.csv")
metadata_file <- file.path(data_dir, "metadata_demo.csv")
annotation_file <- file.path(data_dir, "gene_annotation_demo.csv")
...
############################################################
## 2. Read demo count matrix, metadata and gene annotation
############################################################
...
if (length(missing_cols) > 0) {
  stop(
    paste0(
      "metadata_demo.csv is missing required columns: ",
...
if (length(missing_in_meta) > 0 || length(missing_in_count) > 0) {
  stop("Sample names do not match between demo count matrix and demo metadata.")
}
...
cat("Demo count matrix dimension:\n")
print(dim(count_mat))

cat("Demo group table:\n")
print(table(meta$group))

saveRDS(
  count_mat,
  file = file.path(basic_dir, "gene_counts_demo_matrix.rds")
)

saveRDS(
  meta,
  file = file.path(basic_dir, "metadata_demo_clean.rds")
)

saveRDS(
  gene_anno_gtf,
  file = file.path(basic_dir, "gene_annotation_demo.rds")
)

write.csv(
  meta,
  file = file.path(basic_dir, "metadata_demo_clean.csv"),
  row.names = FALSE,
  quote = FALSE
)
...
write.csv(
  gene_anno_gtf,
  file = file.path(basic_dir, "gene_annotation_demo.csv"),
  row.names = FALSE,
  quote = FALSE
)
...
p_total_counts <- ggplot() +
  ...
  ggtitle("RNA-seq demo")

ggsave(
  filename = file.path(qc_dir, "QC_total_counts_mean_SEM_demo.png"),
  plot = p_total_counts,
  width = 5.5,
  height = 4.5,
  dpi = 300
...
ggsave(
  filename = file.path(qc_dir, "QC_total_counts_mean_SEM_demo.pdf"),
  plot = p_total_counts,
  width = 5.5,
  height = 4.5
)
...
cat("Original demo dds dimension:\n")
print(dim(dds))
...
cat("Filtered demo dds dimension:\n")
print(dim(dds))
...
saveRDS(
  dds,
  file = file.path(basic_dir, "dds_demo_prefiltered.rds")
)

saveRDS(
  vsd,
  file = file.path(basic_dir, "vsd_demo.rds")
)
...
ggsave(
  filename = file.path(pca_dir, "PCA_six_groups_demo.png"),
  plot = p_pca,
  width = 6,
  height = 4,
  dpi = 300
...
ggsave(
  filename = file.path(pca_dir, "PCA_six_groups_demo.pdf"),
  plot = p_pca,
  width = 6,
  height = 4
)
...
write.csv(
  pca_data,
  file = file.path(pca_dir, "PCA_six_groups_demo_data.csv"),
  row.names = TRUE,
  quote = FALSE
)

############################################################
## 6. Top1000 highly variable gene Pearson correlation heatmap
############################################################
...
write.csv(
  group_mean_mat,
  file.path(pearson_dir, "top1000_group_mean_expression_demo.csv"),
  quote = FALSE
)

write.csv(
  pearson_cor,
  file.path(pearson_dir, "top1000_group_mean_Pearson_correlation_demo.csv"),
  quote = FALSE
)


pearson_df <- as.data.frame(as.table(pearson_cor))
colnames(pearson_df) <- c("Group1", "Group2", "Pearson")

pearson_df$Group1 <- factor(
  pearson_df$Group1,
  levels = rev(colnames(pearson_cor))
)

pearson_df$Group2 <- factor(
  pearson_df$Group2,
  levels = colnames(pearson_cor)
)

p_pearson <- ggplot(
  pearson_df,
  aes(x = Group2, y = Group1, fill = Pearson)
) +
  geom_tile(color = "white", linewidth = 0.4) +
  geom_text(
    aes(label = sprintf("%.3f", Pearson)),
    size = 3.2,
    color = "black"
  ) +
...
ggsave(
  filename = file.path(pearson_dir, "top1000_group_mean_Pearson_heatmap_demo.png"),
  plot = p_pearson,
  width = 6.5,
  height = 5.8,
  dpi = 300
...
ggsave(
  filename = file.path(pearson_dir, "top1000_group_mean_Pearson_heatmap_demo.pdf"),
  plot = p_pearson,
  width = 6.5,
  height = 5.8
)
...
############################################################
## 7. DESeq2 pairwise DEG and three-group overall DEG
############################################################
...
for (d in c(deg_unstim_dir, deg_stim_dir, deg_stim_unstim_dir)) {
  dir.create(file.path(d, "DEG_all_bulk"), recursive = TRUE, showWarnings = FALSE)
  dir.create(file.path(d, "DEG_sig_bulk"), recursive = TRUE, showWarnings = FALSE)
}
...
saveRDS(
  dds,
  file = file.path(basic_dir, "dds_demo_DESeq_done.rds")
)

############################################################
## 7.1 Pairwise DEG function
############################################################
...
############################################################
## 7.2 Three-group overall DEG function using LRT
############################################################
...
run_three_group_deg_bulk <- function(dds,
  ...
  ## Demo fallback:
  ## If too few significant genes are available, use the top genes ranked by padj
  ## so that the heatmap module can still be demonstrated.
  if (nrow(res_sig) < 20) {
    message(out_prefix, ": fewer than 20 significant genes in demo data; using top genes by padj for demo heatmap.")
    res_sig <- res_anno[!is.na(res_anno$padj), , drop = FALSE]
    res_sig <- res_sig[order(res_sig$padj, na.last = TRUE), , drop = FALSE]
  }
...
plot_volcano_bulk <- function(res_anno,
  ...
  p <- ggplot(df, aes(x = log2FoldChange, y = neglog10padj)) +
    ...
    theme_bw(base_size = 14) +
    labs(
      title = paste0(out_name, " bulk volcano demo"),
      x = "log2FoldChange",
  ...
  ggsave(
    filename = file.path(outdir, paste0(out_name, "_volcano_bulk_demo.pdf")),
    plot = p,
    width = 7,
    height = 6
  )
  ...
  ggsave(
    filename = file.path(outdir, paste0(out_name, "_volcano_bulk_demo.png")),
    plot = p,
    width = 7,
    height = 6,
    dpi = 300
...
############################################################
## 9. Three-group overall top1000 DEG heatmaps
############################################################
...
plot_three_group_top1000_heatmap_mark_custom_updown <- function(
  ...
  if (is.null(comp_res$res_sig) || nrow(comp_res$res_sig) == 0) {
    message(comp_name, ": res_sig is empty. Heatmap will be skipped.")
    return(NULL)
  }
  ...
  if (nrow(res_df) == 0) {
    message(comp_name, ": no genes after padj filtering. Heatmap will be skipped.")
    return(NULL)
  }
  ...
  if (length(sample_use) == 0) {
    message(comp_name, ": no matched samples. Heatmap will be skipped.")
    return(NULL)
  }
  ...
  if (length(genes_raw_in_mat) == 0) {
    message(comp_name, ": no matched genes. Heatmap will be skipped.")
    return(NULL)
  }
  ...
  write.csv(
    out_table,
    file = file.path(outdir, paste0(comp_name, "_top1000_expression_matrix_demo.csv")),
    row.names = FALSE,
    quote = FALSE
  )
  ...
  write.csv(
    label_table,
    file = file.path(outdir, paste0(comp_name, "_custom_label_genes_in_top1000_demo.csv")),
    row.names = FALSE,
    quote = FALSE
  )
  ...
  pdf(
    file = file.path(outdir, paste0(comp_name, "_top1000_bulk_", file_suffix, "_demo.pdf")),
    width = 6,
    height = 4.7
  )
  ...
  png(
    filename = file.path(outdir, paste0(comp_name, "_top1000_bulk_", file_suffix, "_demo.png")),
    width = 1750,
    height = 1300,
    res = 300
  )
...
############################################################
## 9.1 Unstimulated three-group heatmap
############################################################

up_cell_cycle <- c("CDC25B", "E2F2", "CDK1", "CCNA2", "AURKB")
up_dna_replication <- c("MCM2", "CHEK1", "FEN1", "POLD1", "RFC4")
up_dna_repair <- c("BRCA1", "RAD51", "FANCD2", "XRCC2", "BRIP1")

down_Cytokine_cytokine_receptor_interaction <- c(
  "IL21R", "TNFSF4", "IL12RB2", "EBI3"
)

down_p53_pathway <- c("SESN1", "TP53I3", "GADD45A", "ZMAT3", "CCNG2")
down_chemokine_signaling_pathway <- c("CCR4", "CCR7", "SRC", "PLCG2")

up_genes_unstim <- unique(c(up_cell_cycle, up_dna_replication, up_dna_repair))
...
gene2cat_unstim <- c(
  ...
  setNames(rep("Cytokine-cytokine receptor interaction", length(down_Cytokine_cytokine_receptor_interaction)), down_Cytokine_cytokine_receptor_interaction),
  setNames(rep("p53 pathway", length(down_p53_pathway)), down_p53_pathway),
  setNames(rep("Chemokine signaling pathway", length(down_chemokine_signaling_pathway)), down_chemokine_signaling_pathway)
)
...
heatmap_unstim_3group_overall <- plot_three_group_top1000_heatmap_mark_custom_updown(
  ...
  title_text = "Unstim bulk demo: 3-group overall",
...
############################################################
## 9.2 CD19-stimulated three-group heatmap
############################################################

up_nfkb <- c("CD40", "BTK", "SYK", "BCL2A1", "CXCL8")
up_protein_processing_er <- c("DNAJB11", "HSP90AB1", "SEC62", "LMAN1", "DDIT3")
up_nucleocytoplasmic_transport <- c("IPO5", "KPNA2", "TNPO1", "RAN", "NUP205")

down_chemokine <- c("CCR1", "CCR5", "CXCR6", "JAK3", "GNAI2")
down_nk_cytotoxicity <- c("NCR2", "NCR3", "CD244", "LCK", "FCER1G")
down_integrin_signaling <- c("ITGAL", "ITGB2", "ITGAM", "PIK3CD", "RAC2")
...
gene2cat_stim <- c(
  ...
  setNames(rep("Protein processing in endoplasmic reticulum", length(up_protein_processing_er)), up_protein_processing_er),
  setNames(rep("Nucleocytoplasmic transport", length(up_nucleocytoplasmic_transport)), up_nucleocytoplasmic_transport),
...
heatmap_stim_3group_overall <- plot_three_group_top1000_heatmap_mark_custom_updown(
  ...
  title_text = "Stim bulk demo: 3-group overall",
...
############################################################
## 10. KEGG and GO enrichment analysis
############################################################
...
run_kegg_go_for_deg_results <- function(deg_results_list, outdir_base) {
  ...
  for (comp_name in names(deg_results_list)) {
    ...
    kegg_up <- run_kegg_one_direction(up_info$gene_map, comp_name, "Up")
    kegg_down <- run_kegg_one_direction(down_info$gene_map, comp_name, "Down")
    ...
    summary_df <- dplyr::bind_rows(
      make_enrich_summary_row(comp_name, "Up", "KEGG", "KEGG", length(unique(up_info$gene_vec)), kegg_up$n_mapped, kegg_up$result_df),
      make_enrich_summary_row(comp_name, "Down", "KEGG", "KEGG", length(unique(down_info$gene_vec)), kegg_down$n_mapped, kegg_down$result_df),
...
############################################################
## 11. Selected KEGG pathway visualization
############################################################
## Note for demo analysis:
## The selected KEGG pathways listed below are the same pathway
## terms used in the full original analysis. However, this demo
## dataset contains only a representative subset of genes and is
## intended to test the workflow rather than reproduce the exact
## manuscript-level enrichment results.
##
## Therefore, some selected KEGG pathways may not be present in
## the demo enrichment results. If a selected pathway is not found
## for a given comparison, no bar will be drawn for that pathway.
## Empty KEGG result files will be skipped.
############################################################
...
read_kegg_csv <- function(file) {
  ...
  df <- tryCatch(
    ...
    error = function(e) {
      message("Cannot read KEGG file, skipped: ", basename(file))
      return(data.frame(
  ...
  if (length(miss_cols) > 0) {
    ...
    message(
      "KEGG file has no usable enrichment table and will be skipped: ",
      basename(file)
    )
...
plot_selected_kegg <- function(df_down, df_up, comp_name, outdir) {
  ...
  p <- ggplot(plot_df) +
    ...
    labs(
      title = paste0(comp_name, " KEGG enrichment demo"),
  ...
  out_prefix <- file.path(outdir, paste0(comp_name, "_KEGG_selected_bar_demo"))
...
run_selected_kegg_plot <- function(kegg_dir, selected_kegg_list, outdir) {
  ...
  if (nrow(kegg_all) == 0) {
    message("No usable KEGG enrichment results found in: ", kegg_dir)
    return(NULL)
  }
...
cat("\n========================================\n")
cat("Bulk RNA-seq full demo analysis finished successfully.\n")
...
cat("\nNote:\n")
cat("This demo uses a representative subset of the full dataset.\n")
cat("It is intended to test the workflow and may not reproduce exact manuscript results.\n")
cat("========================================\n")
