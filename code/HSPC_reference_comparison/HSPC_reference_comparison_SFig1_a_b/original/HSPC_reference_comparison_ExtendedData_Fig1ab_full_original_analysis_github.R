############################################################
## HSPC reference comparison full original analysis
## Extended Data Fig. 1a,b
##
## This script performs the full HSPC reference comparison
## analysis using complete Seurat objects.
##
## Main analysis steps:
## 1. Read HSPC wave gene list
## 2. Read adult bone marrow HSPC reference Seurat object
## 3. Read fetal liver HSPC reference Seurat object
## 4. Read teratoma-derived HSPC Seurat object
## 5. Assign reference group labels
...
## Optional fallback if the fetal liver object is still stored as .Rdata
fetal_liver_rdata_file <- file.path(
  input_dir,
...
dir.create(fig_a_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig_b_dir, recursive = TRUE, showWarnings = FALSE)
...
############################################################
## 4. Adult bone marrow HSPC reference
############################################################

cat("Reading adult bone marrow HSPC reference object...\n")
...
############################################################
## 5. Fetal liver HSPC reference
############################################################

cat("Reading fetal liver HSPC reference object...\n")

if (file.exists(fetal_liver_rds_file)) {
  
  seurat.Liver <- readRDS(fetal_liver_rds_file)
  
} else if (file.exists(fetal_liver_rdata_file)) {
  
  load(fetal_liver_rdata_file)
  seurat.Liver <- UpdateSeuratObject(object = sample)
  
} else {
  
  stop("Neither fetal_liver_rds_file nor fetal_liver_rdata_file exists.")
}
...
############################################################
## 6. Teratoma-derived HSPC
############################################################

cat("Reading teratoma-derived HSPC object...\n")
...
## DotPlot displays factor levels from bottom to top.
## Therefore, reverse the levels to make the final figure show:
## FL-HSPC-1, FL-HSPC-2, FL-HSPC-3, FL-HSPC-4, BM-HSPC, Teratoma-HSPC
## from top to bottom.
...
############################################################
## 8. Extended Data Fig. 1a: HSPC wave dot plot
############################################################
...
############################################################
## 9. Pseudo-bulk expression matrix
############################################################
...
############################################################
## 10. Extended Data Fig. 1b: pseudo-bulk hclust
############################################################
...
cat("\n========================================\n")
cat("HSPC reference comparison original analysis finished successfully.\n")
cat("Output directory:\n")
cat(result_dir, "\n\n")
...
cat("1. Adult bone marrow HSPC reference processing\n")
cat("2. Fetal liver HSPC reference processing\n")
cat("3. Teratoma-derived HSPC processing\n")
cat("4. HSPC wave gene dot plot\n")
cat("5. Pseudo-bulk hierarchical clustering\n")
cat("========================================\n")
